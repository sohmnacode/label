...
export function renderNav(profile) {
  ...
  const sections = [
    {
      links: [
        ...
        { route: '/calendar',  icon: iconCal,  label: 'Calendar',  show: true },
      ],
    },
    {
      label: 'Catalog',
    ...
    },
    {
      ...
      links: [
        ...
        { route: '/budget',    icon: iconBudget,   label: 'Budget',    show: isOwnerOrTeam },
      ],
    },
    {
      label: 'Workflow',
  ...
  return `
    <nav class="nav">
      ...
      <div class="nav-links">
        ${sections.map(s => {
          const visible = (s.links || []).filter(l => l.show);
          if (!visible.length) return '';
          return `
            ${s.label ? `<div class="nav-section">${s.label}</div>` : ''}
            ${visible.map(l => `
...
const iconCal      = `<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="1" y="2" width="14" height="13" rx="1"/><line x1="1" y1="6" x2="15" y2="6"/><line x1="5" y1="1" x2="5" y2="4"/><line x1="11" y1="1" x2="11" y2="4"/></svg>`;
...
const iconBudget   = `<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="1" y="4" width="14" height="10" rx="1"/><path d="M5 4V3a3 3 0 016 0v1"/><circle cx="8" cy="9" r="1.5"/></svg>`;
